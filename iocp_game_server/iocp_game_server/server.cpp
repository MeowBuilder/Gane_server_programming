#include <iostream>
#include <WS2tcpip.h>
#include <array>
#include <MSWSock.h>
#include <thread>
#include <vector>
#include "protocol_2026.h"
#include <tbb/concurrent_unordered_map.h>
#include <unordered_set>
#include <mutex>
#include <shared_mutex>
#include <chrono>
#include <concurrent_priority_queue.h>
#include <atomic>

#pragma comment(lib, "MSWSock.lib")
#pragma comment(lib, "WS2_32.lib")
using namespace std;
using namespace std::chrono;

constexpr int BUF_SIZE = 500;
constexpr int VIEW_RANGE = 5;

constexpr int SECTOR_SIZE = 9;
constexpr int SEC_MAX_X = (WORLD_WIDTH / SECTOR_SIZE) + 1;
constexpr int SEC_MAX_Y = (WORLD_HEIGHT / SECTOR_SIZE) + 1;

constexpr int MOVE_COOL_TIME = 1000; // ms
constexpr int EVENT_MOVE = 1;

std::atomic<int> player_index = 0;

struct event_type {
	int obj_id;
	system_clock::time_point wakeup_time;
	int event_id;
	int target_id;

	constexpr bool operator<(const event_type& o) const {
		return wakeup_time > o.wakeup_time;
	}
};
concurrency::concurrent_priority_queue<event_type> timer_queue;

enum IOType { IO_SEND, IO_RECV, IO_ACCEPT, IO_NPC_MOVE };

class EXP_OVER {
public:
	WSAOVERLAPPED m_over;
	IOType  m_iotype;
	WSABUF	m_wsa;
	SOCKET  m_client_socket;
	char    m_buff[BUF_SIZE];
	EXP_OVER()
	{
		ZeroMemory(&m_over, sizeof(m_over));
		m_wsa.buf = m_buff;
		m_wsa.len = BUF_SIZE;
	}
	EXP_OVER(IOType iot) : m_iotype(iot)
	{
		ZeroMemory(&m_over, sizeof(m_over));
		m_wsa.buf = m_buff;
		m_wsa.len = BUF_SIZE;
	}
};

void error_display(const wchar_t* msg, int err_no)
{
	WCHAR* lpMsgBuf;
	FormatMessage(
		FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM,
		NULL, err_no,
		MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
		(LPTSTR)&lpMsgBuf, 0, NULL);
	std::wcout << msg;
	std::wcout << L" === 오류 " << lpMsgBuf << std::endl;
	LocalFree(lpMsgBuf);
}

enum CL_STATE { CS_CONNECT, CS_PLAYING, CS_LOGOUT };

class SESSION {
public:
	SOCKET m_client;
	int m_id;
	CL_STATE m_state;
	EXP_OVER m_recv_over;
	int m_prev_recv;
	char m_username[MAX_NAME_LEN];
	short m_x, m_y;
	int m_move_time;
	std::unordered_set<int> m_visible_players;
	std::mutex m_visible_mutex;
	short m_sec_x, m_sec_y;
	std::atomic<bool> _active_npc{ false };
	system_clock::time_point npc_last_move_time;

	SESSION() {
		std::cout << "SESSION Creation Error!\n";
		exit(-1);
	}
	// Player constructor
	SESSION(SOCKET s, int id) : m_client(s), m_id(id) {
		m_state = CS_CONNECT;
		m_recv_over.m_iotype = IO_RECV;
		m_x = rand() % WORLD_WIDTH;
		m_y = rand() % WORLD_HEIGHT;
		m_sec_x = m_x / SECTOR_SIZE;
		m_sec_y = m_y / SECTOR_SIZE;
		m_prev_recv = 0;
		m_move_time = 0;
		_active_npc = false;
	}
	// NPC constructor (no socket)
	SESSION(int id, bool /*npc_tag*/) : m_client(INVALID_SOCKET), m_id(id) {
		m_state = CS_PLAYING;
		m_x = rand() % WORLD_WIDTH;
		m_y = rand() % WORLD_HEIGHT;
		m_sec_x = m_x / SECTOR_SIZE;
		m_sec_y = m_y / SECTOR_SIZE;
		m_prev_recv = 0;
		m_move_time = 0;
		_active_npc = false;
		snprintf(m_username, sizeof(m_username), "NPC%d", id - NPC_ID_START);
	}
	~SESSION()
	{
		if (m_client != INVALID_SOCKET)
			closesocket(m_client);
	}

	bool is_npc() const { return m_id >= NPC_ID_START; }

	void enter_sector();
	void leave_sector();
	void get_nearby_objects(std::vector<std::shared_ptr<SESSION>>& out);

	// Activate NPC timer (no-op if already active)
	void wake_up();
	// Move NPC randomly and notify nearby players; returns true if nearby players remain
	bool do_random_move();

	bool is_visible(short x, short y) {
		return abs(m_x - x) <= VIEW_RANGE && abs(m_y - y) <= VIEW_RANGE;
	}

	void do_recv()
	{
		DWORD recv_flag = 0;
		memset(&m_recv_over.m_over, 0, sizeof(m_recv_over.m_over));
		m_recv_over.m_wsa.buf = m_recv_over.m_buff + m_prev_recv;
		m_recv_over.m_wsa.len = BUF_SIZE - m_prev_recv;
		WSARecv(m_client, &m_recv_over.m_wsa, 1, 0, &recv_flag, &m_recv_over.m_over, nullptr);
	}
	void do_send(int num_bytes, char* mess)
	{
		EXP_OVER* o = new EXP_OVER(IO_SEND);
		o->m_wsa.len = num_bytes;
		memcpy(o->m_buff, mess, num_bytes);
		WSASend(m_client, &o->m_wsa, 1, 0, 0, &o->m_over, nullptr);
	}
	void send_avatar_info()
	{
		S2C_AvatarInfo packet;
		packet.size = sizeof(S2C_AvatarInfo);
		packet.type = S2C_AVATAR_INFO;
		packet.playerId = m_id;
		packet.x = m_x;
		packet.y = m_y;
		do_send(packet.size, reinterpret_cast<char*>(&packet));
	}
	void send_move_packet(int mover);
	void send_add_player(int player_id);
	void send_login_success()
	{
		S2C_LoginResult packet;
		packet.size = sizeof(S2C_LoginResult);
		packet.type = S2C_LOGIN_RESULT;
		packet.success = true;
		strncpy_s(packet.message, "Login successful.", sizeof(packet.message));
		do_send(packet.size, reinterpret_cast<char*>(&packet));
	}
	void send_remove_player(int player_id)
	{
		S2C_RemovePlayer packet;
		packet.size = sizeof(S2C_RemovePlayer);
		packet.type = S2C_REMOVE_PLAYER;
		packet.playerId = player_id;

		m_visible_mutex.lock();
		if (m_visible_players.count(player_id) == 0) {
			m_visible_mutex.unlock();
			return;
		}
		m_visible_players.erase(player_id);
		m_visible_mutex.unlock();

		do_send(packet.size, reinterpret_cast<char*>(&packet));
	}
	bool process_packet(unsigned char* p);
	void do_move(DIRECTION dir);
};

tbb::concurrent_unordered_map<int, std::atomic<std::shared_ptr<SESSION>>> clients;
tbb::concurrent_unordered_map<int, std::shared_ptr<SESSION>> g_sectors[SEC_MAX_Y][SEC_MAX_X];
std::shared_mutex g_sector_rwlock;

SOCKET g_server;
HANDLE g_iocp;

void SESSION::enter_sector() {
	std::shared_lock<std::shared_mutex> lock(g_sector_rwlock);
	g_sectors[m_sec_y][m_sec_x].insert({ m_id, clients[m_id].load() });
}

void SESSION::leave_sector() {
	std::unique_lock<std::shared_mutex> lock(g_sector_rwlock);
	g_sectors[m_sec_y][m_sec_x].unsafe_erase(m_id);
}

void SESSION::get_nearby_objects(std::vector<std::shared_ptr<SESSION>>& out) {
	std::shared_lock<std::shared_mutex> lock(g_sector_rwlock);
	for (int y = max(0, m_sec_y - 1); y <= min(SEC_MAX_Y - 1, m_sec_y + 1); ++y) {
		for (int x = max(0, m_sec_x - 1); x <= min(SEC_MAX_X - 1, m_sec_x + 1); ++x) {
			for (auto& pair : g_sectors[y][x]) {
				if (pair.first == m_id) continue;
				out.push_back(pair.second);
			}
		}
	}
}

void SESSION::wake_up() {
	bool expected = false;
	if (!_active_npc.compare_exchange_strong(expected, true)) return;

	event_type ev;
	ev.obj_id = m_id;
	ev.event_id = EVENT_MOVE;
	ev.target_id = -1;
	ev.wakeup_time = system_clock::now() + milliseconds(MOVE_COOL_TIME);
	timer_queue.push(ev);
}

bool SESSION::do_random_move() {
	// Collect visible players before move
	std::vector<std::shared_ptr<SESSION>> nearby;
	get_nearby_objects(nearby);

	std::unordered_set<int> old_v;
	for (auto& pl : nearby) {
		if (!pl || pl->m_state != CS_PLAYING || pl->is_npc()) continue;
		if (is_visible(pl->m_x, pl->m_y))
			old_v.insert(pl->m_id);
	}

	// Random move
	short old_sec_x = m_sec_x, old_sec_y = m_sec_y;
	switch (rand() % 4) {
	case 0: m_y = max(0, m_y - 1); break;
	case 1: m_y = min(WORLD_HEIGHT - 1, m_y + 1); break;
	case 2: m_x = max(0, m_x - 1); break;
	case 3: m_x = min(WORLD_WIDTH - 1, m_x + 1); break;
	}
	m_sec_x = m_x / SECTOR_SIZE;
	m_sec_y = m_y / SECTOR_SIZE;

	if (old_sec_x != m_sec_x || old_sec_y != m_sec_y) {
		std::unique_lock<std::shared_mutex> lock(g_sector_rwlock);
		g_sectors[old_sec_y][old_sec_x].unsafe_erase(m_id);
		g_sectors[m_sec_y][m_sec_x].insert({ m_id, clients[m_id].load() });
	}

	// Collect visible players after move
	nearby.clear();
	get_nearby_objects(nearby);

	std::unordered_set<int> new_v;
	for (auto& pl : nearby) {
		if (!pl || pl->m_state != CS_PLAYING || pl->is_npc()) continue;
		if (is_visible(pl->m_x, pl->m_y))
			new_v.insert(pl->m_id);
	}

	// Notify players
	for (int id : new_v) {
		std::shared_ptr<SESSION> pl = clients[id].load();
		if (!pl) continue;
		if (old_v.count(id) == 0)
			pl->send_add_player(m_id);   // NPC newly entered player's view
		else
			pl->send_move_packet(m_id);  // NPC moved within player's view
	}
	for (int id : old_v) {
		if (new_v.count(id) == 0) {
			std::shared_ptr<SESSION> pl = clients[id].load();
			if (!pl) continue;
			pl->send_remove_player(m_id);  // NPC left player's view
		}
	}

	return !new_v.empty();
}

void SESSION::send_add_player(int player_id)
{
	S2C_AddPlayer packet;
	packet.size = sizeof(S2C_AddPlayer);
	packet.type = S2C_ADD_PLAYER;
	packet.playerId = player_id;
	std::shared_ptr<SESSION> pl = clients[player_id].load();
	if (nullptr == pl) return;
	memcpy(packet.username, pl->m_username, sizeof(packet.username));
	packet.x = pl->m_x;
	packet.y = pl->m_y;

	m_visible_mutex.lock();
	if (m_visible_players.count(player_id) > 0) {
		m_visible_mutex.unlock();
		return;
	}
	m_visible_players.insert(player_id);
	m_visible_mutex.unlock();

	do_send(packet.size, reinterpret_cast<char*>(&packet));
}

void SESSION::do_move(DIRECTION dir)
{
	std::unordered_set<int> old_v;
	{
		std::lock_guard<std::mutex> lock(m_visible_mutex);
		old_v = m_visible_players;
	}
	short old_sec_x = m_sec_x;
	short old_sec_y = m_sec_y;

	switch (dir) {
	case UP:    m_y = max(0, m_y - 1); break;
	case DOWN:  m_y = min(WORLD_HEIGHT - 1, m_y + 1); break;
	case LEFT:  m_x = max(0, m_x - 1); break;
	case RIGHT: m_x = min(WORLD_WIDTH - 1, m_x + 1); break;
	}

	m_sec_x = m_x / SECTOR_SIZE;
	m_sec_y = m_y / SECTOR_SIZE;

	if (old_sec_x != m_sec_x || old_sec_y != m_sec_y) {
		std::unique_lock<std::shared_mutex> lock(g_sector_rwlock);
		g_sectors[old_sec_y][old_sec_x].unsafe_erase(m_id);
		g_sectors[m_sec_y][m_sec_x].insert({ m_id, clients[m_id].load() });
	}

	std::vector<std::shared_ptr<SESSION>> nearby;
	get_nearby_objects(nearby);

	std::unordered_set<int> new_v;
	for (auto& pl : nearby) {
		if (pl == nullptr) continue;
		if (pl->m_state != CS_PLAYING) continue;
		if (is_visible(pl->m_x, pl->m_y))
			new_v.insert(pl->m_id);
	}

	send_move_packet(m_id);

	for (int id : new_v) {
		if (old_v.count(id) == 0) {
			// Newly visible object
			send_add_player(id);
			std::shared_ptr<SESSION> pl = clients[id].load();
			if (nullptr == pl) continue;
			if (pl->is_npc()) {
				pl->wake_up(); // activate NPC that entered view
			} else {
				pl->send_add_player(m_id); // mutual awareness with other player
			}
		} else {
			// Still visible: only notify other players (NPCs handle their own move notifications)
			std::shared_ptr<SESSION> pl = clients[id].load();
			if (nullptr == pl) continue;
			if (!pl->is_npc())
				pl->send_move_packet(m_id);
		}
	}

	for (int id : old_v) {
		if (new_v.count(id) == 0) {
			// No longer visible
			send_remove_player(id);
			std::shared_ptr<SESSION> pl = clients[id].load();
			if (nullptr == pl) continue;
			if (!pl->is_npc())
				pl->send_remove_player(m_id);
		}
	}
}

bool SESSION::process_packet(unsigned char* p)
{
	PACKET_TYPE type = *reinterpret_cast<PACKET_TYPE*>(&p[1]);
	switch (type) {
	case C2S_LOGIN: {
		C2S_Login* packet = reinterpret_cast<C2S_Login*>(p);
		strncpy_s(m_username, packet->username, MAX_NAME_LEN);
		cout << "Player[" << m_id << "] logged in as " << m_username << "\n";
		send_avatar_info();
		m_state = CS_PLAYING;
		enter_sector();

		// Use sector search to find nearby objects (efficient with large NPC count)
		std::vector<std::shared_ptr<SESSION>> nearby;
		get_nearby_objects(nearby);
		for (auto& pl : nearby) {
			if (nullptr == pl) continue;
			if (pl->m_state != CS_PLAYING) continue;
			if (!is_visible(pl->m_x, pl->m_y)) continue;
			send_add_player(pl->m_id);
			if (pl->is_npc()) {
				pl->wake_up(); // activate visible NPC on login
			} else {
				pl->send_add_player(m_id);
			}
		}
		break;
	}
	case C2S_MOVE: {
		C2S_Move* packet = reinterpret_cast<C2S_Move*>(p);
		DIRECTION dir = packet->dir;
		m_move_time = packet->move_time;
		do_move(dir);
		break;
	}
	default:
		cout << "Unknown packet type received from player[" << m_id << "].\n";
		return false;
	}
	return true;
}

void SESSION::send_move_packet(int mover)
{
	S2C_MovePlayer packet;
	packet.size = sizeof(S2C_MovePlayer);
	packet.type = S2C_MOVE_PLAYER;
	packet.playerId = mover;
	std::shared_ptr<SESSION> pl = clients[mover].load();
	if (nullptr == pl) return;
	packet.x = pl->m_x;
	packet.y = pl->m_y;
	packet.move_time = pl->m_move_time;
	do_send(packet.size, reinterpret_cast<char*>(&packet));
}

void send_login_fail(SOCKET client, const char* message)
{
	S2C_LoginResult packet;
	packet.size = sizeof(S2C_LoginResult);
	packet.type = S2C_LOGIN_RESULT;
	packet.success = false;
	strncpy_s(packet.message, message, sizeof(packet.message));
	WSABUF wsa_buf;
	wsa_buf.buf = reinterpret_cast<char*>(&packet);
	wsa_buf.len = packet.size;
	WSASend(client, &wsa_buf, 1, 0, 0, nullptr, nullptr);
}

void disconnect(int key)
{
	std::cout << "client[" << key << "] Disconnected.\n";
	std::shared_ptr<SESSION> cl = clients[key].load();
	if (nullptr != cl) {
		cl->leave_sector();
		cl->m_state = CS_LOGOUT;
		std::unordered_set<int> visible_copy;
		{
			std::lock_guard<std::mutex> lock(cl->m_visible_mutex);
			visible_copy = cl->m_visible_players;
		}
		for (auto& other : visible_copy) {
			std::shared_ptr<SESSION> o = clients[other].load();
			if (nullptr == o) continue;
			if (o->is_npc()) continue; // NPCs don't receive remove packets
			if (CS_PLAYING == o->m_state)
				o->send_remove_player(key);
		}
		closesocket(cl->m_client);
		cl->m_client = INVALID_SOCKET;
	}
	clients[key].store(nullptr);
}

void InitializeNPC()
{
	cout << "NPC initialize begin.\n";
	for (int i = NPC_ID_START; i < NPC_ID_START + MAX_NPCS; ++i) {
		auto npc = std::make_shared<SESSION>(i, true);
		clients[i] = npc;
		npc->enter_sector();
	}
	cout << "NPC initialize end. (" << MAX_NPCS << " NPCs created)\n";
}

void timer_thread()
{
	while (true) {
		event_type ev;
		if (timer_queue.try_pop(ev)) {
			auto now = system_clock::now();
			if (ev.wakeup_time <= now) {
				if (ev.event_id == EVENT_MOVE) {
					EXP_OVER* move_over = new EXP_OVER(IO_NPC_MOVE);
					PostQueuedCompletionStatus(g_iocp, 1, ev.obj_id, &move_over->m_over);
				}
			} else {
				// Not ready yet, put it back
				timer_queue.push(ev);
				this_thread::sleep_for(chrono::milliseconds(1));
			}
		} else {
			this_thread::sleep_for(chrono::milliseconds(1));
		}
	}
}

void worker_thread()
{
	for (;;) {
		DWORD num_bytes;
		ULONG_PTR long_key;
		LPOVERLAPPED over;
		BOOL ret = GetQueuedCompletionStatus(g_iocp, &num_bytes, &long_key, &over, INFINITE);
		int key = static_cast<int>(long_key);
		if (TRUE != ret) {
			error_display(L"GQCS Error: ", WSAGetLastError());
			if (key == -1) exit(-1);
			disconnect(key);
			continue;
		}
		EXP_OVER* exp_over = reinterpret_cast<EXP_OVER*>(over);
		switch (exp_over->m_iotype) {
		case IO_ACCEPT: {
			int my_id = player_index++;
			if (my_id >= NPC_ID_START) {
				cout << "Server full, rejecting connection.\n";
				closesocket(exp_over->m_client_socket);
				exp_over->m_client_socket = WSASocket(AF_INET, SOCK_STREAM, 0, NULL, 0, WSA_FLAG_OVERLAPPED);
				AcceptEx(g_server, exp_over->m_client_socket, &exp_over->m_buff, 0,
					sizeof(SOCKADDR_IN) + 16, sizeof(SOCKADDR_IN) + 16,
					NULL, &exp_over->m_over);
				break;
			}
			CreateIoCompletionPort((HANDLE)exp_over->m_client_socket, g_iocp, my_id, 0);
			std::shared_ptr<SESSION> new_pl = std::make_shared<SESSION>(exp_over->m_client_socket, my_id);
			clients[my_id] = new_pl;
			new_pl->send_login_success();
			new_pl->do_recv();
			exp_over->m_client_socket = WSASocket(AF_INET, SOCK_STREAM, 0, NULL, 0, WSA_FLAG_OVERLAPPED);
			AcceptEx(g_server, exp_over->m_client_socket, &exp_over->m_buff, 0,
				sizeof(SOCKADDR_IN) + 16, sizeof(SOCKADDR_IN) + 16,
				NULL, &exp_over->m_over);
			break;
		}
		case IO_RECV:
		{
			if (0 == num_bytes) {
				disconnect(key);
				break;
			}
			std::shared_ptr<SESSION> cl = clients[key].load();
			if (nullptr == cl) break;
			unsigned char* p = reinterpret_cast<unsigned char*>(cl->m_recv_over.m_buff);
			int data_size = num_bytes + cl->m_prev_recv;
			bool do_disconnect = false;
			while (data_size > 0) {
				int packet_size = p[0];
				if (packet_size == 0 || packet_size > data_size) break;
				if (false == cl->process_packet(p)) {
					do_disconnect = true;
					break;
				}
				p += packet_size;
				data_size -= packet_size;
			}
			if (do_disconnect) {
				disconnect(key);
				break;
			}
			if (data_size > 0)
				memmove(cl->m_recv_over.m_buff, p, data_size);
			cl->m_prev_recv = data_size;
			cl->do_recv();
			break;
		}
		case IO_SEND: {
			delete exp_over;
			break;
		}
		case IO_NPC_MOVE: {
			delete exp_over;
			int npc_id = key;
			std::shared_ptr<SESSION> npc = clients[npc_id].load();
			if (nullptr == npc) break;

			bool has_nearby = npc->do_random_move();

			if (has_nearby) {
				// Re-queue next move event for this NPC
				event_type ev;
				ev.obj_id = npc_id;
				ev.event_id = EVENT_MOVE;
				ev.target_id = -1;
				ev.wakeup_time = system_clock::now() + milliseconds(MOVE_COOL_TIME);
				timer_queue.push(ev);
			} else {
				// No players nearby: deactivate NPC until a player approaches
				npc->_active_npc = false;
			}
			break;
		}
		default:
			cout << "Unknown IO type." << endl;
			exit(-1);
		}
	}
}

int main()
{
	WSADATA WSAData;
	WSAStartup(MAKEWORD(2, 2), &WSAData);
	g_server = WSASocket(AF_INET, SOCK_STREAM, 0, NULL, 0, WSA_FLAG_OVERLAPPED);
	SOCKADDR_IN server_addr;
	memset(&server_addr, 0, sizeof(server_addr));
	server_addr.sin_family = AF_INET;
	server_addr.sin_port = htons(PORT);
	server_addr.sin_addr.S_un.S_addr = INADDR_ANY;
	::bind(g_server, reinterpret_cast<sockaddr*>(&server_addr), sizeof(server_addr));
	listen(g_server, SOMAXCONN);
	g_iocp = CreateIoCompletionPort(INVALID_HANDLE_VALUE, NULL, 0, 0);
	CreateIoCompletionPort((HANDLE)g_server, g_iocp, -1, 0);

	InitializeNPC();

	EXP_OVER accept_over(IO_ACCEPT);
	accept_over.m_client_socket = WSASocket(AF_INET, SOCK_STREAM, 0, NULL, 0, WSA_FLAG_OVERLAPPED);
	AcceptEx(g_server, accept_over.m_client_socket, &accept_over.m_buff, 0,
		sizeof(SOCKADDR_IN) + 16, sizeof(SOCKADDR_IN) + 16,
		NULL, &accept_over.m_over);

	thread timer_th(timer_thread);

	vector<thread> worker_threads;
	int num_threads = thread::hardware_concurrency();
	for (int i = 0; i < num_threads; ++i)
		worker_threads.emplace_back(worker_thread);
	for (auto& th : worker_threads)
		th.join();
	timer_th.join();

	closesocket(g_server);
	WSACleanup();
}
