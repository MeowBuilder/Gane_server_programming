...
public:
	...
	void send_chat_packet(int sender, char* message)
	{
		S2C_ChatMessage packet;
		packet.size = sizeof(S2C_ChatMessage) - MAX_CHAT_MSG_LEN + strlen(message) + 1;
		packet.type = S2C_CHAT_MESSAGE;
		packet.object_id = sender;
		strncpy_s(packet.message, message, sizeof(message));
		do_send(packet.size, reinterpret_cast<char*>(&packet));
	}
...
int API_SendMessage(lua_State* L)
{
	...
	if (nullptr == pl) {
		...
		pl->send_chat_packet(my_id, mess);
	}
	return 0;
}
